...
| **_relationMetadata**    | An object field populated only on **through-query endpoints** — never on direct endpoints (where it is excluded from the response schema). On `GET /lists/{id}/entities` and `GET /entities/{id}/lists` it contains join-table metadata (`_id`, `_kind`, `_validFromDateTime`, `_validUntilDateTime`) from the `ListToEntityRelation` record that links the returned document to the queried parent. On `GET /entities/{id}/reactions` and `GET /lists/{id}/reactions` it contains source and target metadata describing the record that owns the reaction. Present in entity, list, entity-reaction, and list-reaction models. |
