...
export class SetFilterBuilder<T extends object = AnyObject> {
  ...
  build(): Filter<T> {
    ...
    const setWhere: Where<T>[] | Where<T> = this.buildWhereClauseForConditions(this.set, keys);
  ...
  buildWhereClauseForSingleKeyOfSet(parentSet: Set, key: string): Where<AnyObject>[] | Where<AnyObject> {

    if (key !== 'and' && key !== 'or')
      return this.setTransformer.produceWhereClauseFor(key, _.get(parentSet, key));
  ...
  buildWhereClauseForConditions(parentSet: Set, keys: string[]): Where<AnyObject>[] | Where<AnyObject> {
...
class SetToFilterTransformer {
  ...
  constructor(private _ownerUsers?: string[], private _ownerGroups?: string[]) {
  }

  produceWhereClauseFor(setName: string, setValue?: string | UserAndGroupInfo): Where<AnyObject> {
