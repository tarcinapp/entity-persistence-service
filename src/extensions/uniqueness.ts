...
export class UniquenessConfigurationReader {
  ...
  constructor() {
    ...
    if (_.isString(process.env.uniqueness_entity_fields)) {
      ...
      // prevent parsing fields string for multiple times
      if (!UniquenessConfigurationReader.COMMON_ENTITY_UNIQUENESS_FIELDS)
  ...
  public isUniquenessConfiguredForLists(kind?: string): boolean {
    ...
    return (_.isString(process.env.uniqueness_list_fields)
  ...
  public getSetForLists(kind?: string): string {
    return "";
  }
}
